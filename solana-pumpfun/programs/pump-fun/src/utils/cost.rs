...
pub fn calculate_cost(current_supply: u64, tokens_to_buy: u64, decimals: u8, token_k: f64, token_initial_price: u64) -> u64 {
    // // Calculate the exponent parts scaled to avoid precision loss
    let exponent1 = token_k * convert_to_float(current_supply + tokens_to_buy, decimals);
    let exponent2 = token_k * convert_to_float(current_supply, decimals);
    ...
    let cost = ((token_initial_price as f64) * (exp1 - exp2)) / token_k; // Adjust for k scaling without dividing by zero
    return cost as u64;
}

pub fn calculate_token_amount(current_supply: u64, sol_amount: u64, decimals: u8, token_k: f64, token_initial_price: u64) -> u64 {
    // k * currentSupply
    let exponent = token_k * convert_to_float(current_supply, decimals);

    // Calculate e^(k * currentSupply)
    let exp1 = exponent.exp();
    // Calculate (solAmount * K) / INIT_PRICE
    let num = ((sol_amount as f64) * token_k) / (token_initial_price as f64);
    // Calculate ln((solAmount * K) / INIT_PRICE + e^(k * currentSupply))
    let ln = (num + exp1).ln();
    // formula: (ln((ethAmount * K) / INIT_PRICE + e^(k * currentSupply)) / K) - currentSupply
    let token_amount = convert_from_float(ln / token_k , decimals) - current_supply;
    return token_amount;
}
...
mod tests {
    ...
    fn test_cost() {
        ...
        let result = calculate_cost(current_supply, tokens_to_buy, 6, DEFAULT_K, DEFAULT_INITIAL_PRICE);
    ...
    fn test_calculate_token_amount() {
        ...
        let result = calculate_token_amount(current_supply, sol_amount, 6, DEFAULT_K, DEFAULT_INITIAL_PRICE);
